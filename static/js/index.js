$(function () {
      ...
      .done(function (res) {
        ...
        // // 需要修改 href  接收后台传来的id号 响应详情页  /news/${one_news.id}/
          res.data.news.forEach(function (one_news) {
            let content = `
              <li class="news-item">
                 <a href="/news/news/${one_news.id}/" class="news-thumbnail" target="_blank">
                    <img src="${one_news.image_url}" alt="${one_news.title}" title="${one_news.title}">
                 </a>
                 <div class="news-content">
                    <h4 class="news-title"><a href="/news/news/${one_news.id}/" target="_blank">${one_news.title}</a></h4>
  ...
  function fn_load_banner() {
    $.ajax({
      // 请求地址
      url: "/news/news/banners/",  // url尾部需要添加/
      // 请求方式
      type: "GET",
      async: false
    })
        ...
        if (res.errno === "0") {
          ...
          res.data.banners.forEach(function (one_banner, index) {
            if (index === 0){
              ...
              content = `
                <li style="display:block;"><a href="/news/news/${one_banner.news_id}/" target="_blank">
                 <img src="${one_banner.image_url}" alt="${one_banner.news_title}"></a></li>
              `;
              tab_content = `<li class="active"></li>`;
            } else {
              content = `
              <li><a href="/news/news/${one_banner.news_id}/"><img src="${one_banner.image_url}" alt="${one_banner.news_title}"></a></li>
              `;
              tab_content = `<li></li>`;
            }
